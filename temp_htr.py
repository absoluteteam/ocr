...
def emnist_model(labels_num=None):
    model = Sequential()
    model.add(Convolution2D(filters=32, kernel_size=(3, 3), padding='valid', input_shape=(28, 28, 1),
                            activation='relu'))
    model.add(Convolution2D(filters=64, kernel_size=(3, 3), activation='relu'))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    # Отключение четверти случайных нейронов на слое для улучшения распознаваемости
    model.add(Dropout(0.25))
    model.add(Flatten())
    model.add(Dense(512, activation='relu'))
    # Отключение половины случайных нейронов на слое для улучшения распознаваемости
...
if epochs != -1:
    ...
    for i in chars:
        for t, j in enumerate(os.listdir(f'Cyrillic/{i}')):
            ...
            if 0 <= (ord(i) - ord('А')) <= 31:
                ...
                # 1 будет стоять у элемента с индексом, совпадающим с буквой ответа для данного теста
    ...
    except Union[ImportError, IOError]:
    ...
    letters = letters_extract(sys.argv[1], 28)
    for i in letters:
        model = keras.models.load_model('model')
        # Изменение измерений входных данных
